...
const token = require("../security/jwt");
  ...
  try {
    ...
    if (find.length > 0) {
      let was_found = encryptObject({
        ...
        error: false,
        message:
          "user signed with this id before if you forget your password contact admin inorder to reset your password",
      });
      res.send(was_found);
    } else {
      /**
...
const Login = async (req, res) => {
  try {
    ...
    if (!user.length ? true : false) {
      let secure = encryptObject({
        login: false,
        error: false,
        message: "sorry unknown employee id",
      });
      ...
      if (compare) {
        ...
        if (!signed) throw new error("authenticating with token failed");
        //encrypting response
        let encrypt = encryptObject({
          login: true,
          error: false,
    ...
    console.log(err);
    let errorSecured = encryptObject({
      login: false,
      error: true,
      message: err,
    });
...
const deleteUser = async (req, res) => {
  try {
    ...
    let { _id } = decrpt;
    if (_id ? false : true) throw new Error("_id is not set");
